...
// Our assembly function (from previous code)
extern void vector_int_add_rvv(const int32_t *a, const int32_t *b, int32_t *c, size_t n);
...
int main(void) {
    ...
    double elapsed_sec;
    const int N = 32;
    int32_t a[N], b[N], c[N];    // 32-bit integer data
    int steps=1000000;
    ...
    for(int i=0;i<steps;i++) vector_int_add_rvv(a, b, c, N);
    endv=clock();
